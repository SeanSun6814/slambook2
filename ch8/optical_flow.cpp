...
int main(int argc, char** argv) {
    ...
    Mat img1;
    Mat img2;
    VideoCapture capture("http://172.23.32.1:8000/camera/mjpeg");

    Mat newImage;
    capture.read(newImage);
    cv::cvtColor(newImage, img1, cv::COLOR_BGR2GRAY);

    while (true) {
        if (!capture.read(newImage))
            break;
        // img2 = img1;
        // img1 = newImage;
        cv::cvtColor(newImage, img2, cv::COLOR_BGR2GRAY);
